			...
			link    : function(originalScope, element, attrs, modelCtrl) {
				...
				function getHeaderText() {
					if(isEmpty(modelCtrl.$modelValue)) {
						scope.header = attrs.msHeader || "Selecione de la lista  ";
						return scope.header;
					}
					...
					if(isMultiple) {
						if(attrs.msSelected) {
							...
							scope.header = modelCtrl.$modelValue.length + " " + "seleccionado(s)";
						}
...
angular.module("multiselect.tpl.html", []).run(["$templateCache", function($templateCache) {
	$templateCache.put("multiselect.tpl.html",
			...
			"    <input ng-show=\"items.length > filterAfterRows\" ng-model=\"filter\" style=\"padding: 0px 3px;margin-right: 15px; margin-bottom: 4px;\" placeholder=\"Buscar\">" +
